...
public class GameScene implements GLEventListener {
    ...
    public void drawBall(GL2 gl, float xPosition, float yPosition) {
        ...
        gl.glColor3f(0, 0, 0); // Cor preta para a bomba

        // Desenhar a parte superior da bomba
        ...
        // Desenhar a parte inferior da bomba
        gl.glBegin(GL2.GL_POLYGON);
        for (int z = 0; z < vertices; z++) {
            angle = z * angleIncrement;
            double x = 0.7 * radius * Math.cos(angle); // Aumenta a curvatura para criar a parte inferior da bomba
            double y = radius * Math.sin(angle);
            gl.glVertex2d(x, y);
        }
        gl.glEnd();

        // Desenhar o pavio da bomba
        gl.glColor3f(1, 0.5f, 0); // Cor laranja para o pavio aceso
        gl.glLineWidth(2.0f); // Reduzir a largura do pavio
        gl.glBegin(GL2.GL_LINES);
        gl.glVertex2d(0, radius); // Extremidade inferior do pavio
        gl.glVertex2d(0, radius + 0.3); // Extremidade superior do pavio (reduzi para 0.3)
        gl.glEnd();
        gl.glLineWidth(1.0f); // Restaurar a largura padrão
    ...
    public void drawTriangle(GL2 gl) {
        ...
        gl.glColor3f(1, 1, 1); // Cor branca para o triângulo

        gl.glBegin(GL2.GL_TRIANGLES);
        gl.glVertex2f(0, 1); // Top vertex
        gl.glVertex2f(-1, -1); // Bottom-left vertex
        gl.glVertex2f(1, -1); // Bottom-right vertex
        gl.glEnd();
    ...
    public void display(GLAutoDrawable drawable) {
        ...
        if (Pong.level == 2) {
            drawTriangle(gl); // Desenha o objeto da segunda fase
        }
        ...
        if (Pong.gameOver) {
            ...
            TextUtils.drawText(drawable, "BOOOOOOOM!!!!!!!!", 30, width / 2, height / 2);
        }
